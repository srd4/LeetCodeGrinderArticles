...
'Vértices' y 'ejes' son la nomenclatura con la que te encuentras en problemas que hacen uso de grafos de cualquier tipo. Pero también, curiosamente, puedo relacionar esto con la matemática o geometría de nivel elemental, donde hablan de vértices y ejes/aristas de figuras geométricas también. Todo en la vida conecta, de una manera u otra.

Hay muchos problemas que te hacen pensar en aristas y vértices. Recuerdo muchas veces estar confundido acerca de lo que significaban por vértices y aristas y, de alguna manera, confundir uno con el otro. Pero ahora es bastante rápido entender de qué habla la gente en mi cabeza mientras leo. Supongo que la comprensión lectora en cuanto a ese tipo de contexto y vocabulario ha mejorado en mí.

Y sí, hay muchos problemas sobre eso. Así que si al principio te sientes algo lento con ellos o con su nomenclatura, sigue esforzándote, no es que se vuelvan más fáciles, sino que tú te vuelves más duro.

También hay muchos problemas básicamente sobre tener que rodear o enumerar posibilidades. De hecho, hay una etiqueta literal de Enumeración donde tienes que descubrir y listar cada uno de los casos límite del problema y decidir cómo manejarlos explícitamente.

En este caso es conceptualmente algo similar porque descubres que cada mono puede moverse en una dirección u otra en un paso. No pueden ir más lejos de una vez. Así que esas opciones de movimientos son los casos que enumeramos, contabilizamos y manejamos en nuestro código. Luego tenemos que pensar en las posibles combinaciones de eso.

Si no hay solo un mono, sino dos monos o tres o cuatro, y el número de monos depende del número de vértices en el polígono y cada uno tiene la capacidad de tomar una decisión u otra, eso junto hace un conjunto de combinaciones de decisiones finales teniendo en cuenta lo que cada mono decide hacer.

Así que, en resumen, enfrentándome a LeetCode he encontrado problemas sobre grafos, y cosas que tienen que ver con la enumeración y explícitamente con cosas binarias: cadenas binarias y manipulación de bits, otros comunes. El primero incluso si son solo problemas geométricos fáciles como este o problemas complejos en los que aún tienes que pensar de manera abstracta sobre grafos y aristas mientras trabajas con arreglos.
